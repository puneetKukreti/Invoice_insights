...
const compareCharges = (invoice: ExtractedData, rates: QuotationRates | null): ExtractedData['comparisonStatus'] => {
  if (!rates) return 'no_quotation_data'; // This case should ideally not be hit if rates are always hardcoded
  ...
  if (invoice.shipmentType === 'air') {
    ...
    // If both rates are undefined, consider it not comparable for specific coloring
    if (rates.airServiceChargeRate === undefined && rates.airLoadingChargeRate === undefined && (invoiceServiceActual > 0 || invoiceLoadingActual > 0)) {
        return 'not_comparable_charges';
    }
    ...
    // If both rates are undefined, consider it not comparable for specific coloring
     if (rates.oceanServiceChargeRate === undefined && rates.oceanLoadingChargeRate === undefined && (invoiceServiceActual > 0 || invoiceLoadingActual > 0)) {
        return 'not_comparable_charges';
    }
  }
...
export function InvoiceDataTable() {
  ...
  const handleExport = () => {
    ...
    const headers = [
     ...
     const dataForSheet = dataToDisplay.map(item => ({
        ...
        "Cargomen Own Charges(Loading,unloading,Agency Charges,Transportation If any)": item.cargomenOwnCharges.toFixed(2),
        "REIMBURSEMENT Charges(Storage Charge,Do charges,Celebi ..ect)": item.reimbursementCharges.toFixed(2),
        "Total Invoice Value": (item.cargomenOwnCharges + item.reimbursementCharges).toFixed(2),
        "Source File": item.filename || 'N/A',
    }));

    const worksheet = XLSX.utils.json_to_sheet(dataForSheet, { header: headers }); // Pass headers explicitly
    XLSX.utils.sheet_add_json(worksheet, dataForSheet, { origin: "A2", skipHeader: true }); // Append data starting from A2
    ...
    for (let R = range.s.r; R <= range.e.r; ++R) {
      for (let C = range.s.c; C <= range.e.c; ++C) {
        ...
        // Example: Wrap text for specific columns based on header names
        const headerName = headers[C];
        if (headerName === "Cargomen Own Charges(Loading,unloading,Agency Charges,Transportation If any)" || headerName === "REIMBURSEMENT Charges(Storage Charge,Do charges,Celebi ..ect)" || headerName === "Source File") {
            if (!worksheet[cellref].s.alignment) worksheet[cellref].s.alignment = {};
            worksheet[cellref].s.alignment.wrapText = true;
        }
      }
    }
    worksheet['!cols'] = [ {wch: 15}, {wch: 15}, {wch: 15}, {wch: 15}, {wch: 15}, {wch: 20}, {wch: 15}, {wch: 20}, {wch: 20}, {wch: 25}, {wch: 40}, {wch: 40}, {wch: 20}, {wch: 30}];
  ...
  const getStatusIcon = (status?: ExtractedData['comparisonStatus']) => {
    switch (status) {
      ...
      case 'no_quotation_data': // Should not be hit if quotation is hardcoded
  ...
  const getStatusTooltip = (status?: ExtractedData['comparisonStatus']) => {
    switch (status) {
      ...
      case 'no_quotation_data': // Should not be hit
        return "Standard quotation data is being used for comparison.";
      case 'invoice_type_unknown':
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Cargomen Own Charges(Loading,unloading,Agency Charges,Transportation If any)</TableHead>
              <TableHead className="text-right">REIMBURSEMENT Charges(Storage Charge,Do charges,Celebi ..ect)</TableHead>
              <TableHead className="text-right font-bold">Total Invoice Value</TableHead>
            ...
            {dataToDisplay.length === 0 ? (
              <TableRow>
                <TableCell colSpan={14} className="h-24 text-center text-muted-foreground">
                  Upload invoices to see the extracted data and comparison with the standard quotation.
                </TableCell>
              </TableRow>
            ) : (
