...
/**
 * @fileOverview Classifies invoice charges into 'Cargomen Own Charges' and 'Reimbursement Charges' directly from an invoice PDF, summing the total amounts including tax from the 'Total (INR)' column.
 *
...
const ClassifyInvoiceChargesOutputSchema = z.object({
  cargomenOwnCharges: z.number().describe('The sum of the "Total (INR)" values (including tax) for strictly "SERVICE CHARGES", "LOADING & UNLOADING CHARGES", and "TRANSPORTATION" taken directly from the "Total (INR)" column.').optional().default(0),
  reimbursementCharges: z.number().describe('The sum of the "Total (INR)" values (including tax) for all other charges not classified as Cargomen Own Charges, taken directly from the "Total (INR)" column.').optional().default(0),
});
...
const classifyInvoiceChargesPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert invoice processing specialist for Cargomen. Your task is to meticulously analyze the charge breakdown section (usually a table with columns like 'Description', 'Taxable Value', 'IGST', 'Total (INR)') of the provided invoice document. You need to classify each individual charge line item into ONLY one of two categories based on its description, and then sum the values **ONLY** from the **"Total (INR)"** column for each category according to these VERY SPECIFIC rules:
  ...
  **Definitions:**
  ...
  2.  **Reimbursement Charges**: These are ALL OTHER costs listed as individual line items in the charge breakdown section that are NOT one of the three specific "Cargomen Own Charges" listed above. Sum the amounts ONLY from the **"Total (INR)"** column for these items. This includes, but is not limited to:
  ...
  *   **DO NOT include the overall invoice subtotal, total tax, or grand total.** Only sum the values found in the **"Total (INR)"** column within the charge breakdown table itself for individual line items.
  ...
  *   Double-check your classifications against the strict rules above and ensure your arithmetic sums using ONLY the **"Total (INR)"** values are accurate.
  ...
  *   Ensure the output values are valid JSON numbers, not strings or formatted currency.
  ...
  Provide the final *total sum* for \`cargomenOwnCharges\` and the final *total sum* for \`reimbursementCharges\` (both derived strictly from the "Total (INR)" column) as a JSON object matching the output schema. Example based on the values above: {"cargomenOwnCharges": 12060.00, "reimbursementCharges": 1516.00}
  `,
});
...
>(
  ...
  async input => {
    console.log("Calling classifyInvoiceChargesPrompt with PDF data URI (expecting total incl. tax from 'Total (INR)' column)...");
    ...
    console.log(`Parsed charges (from Total (INR)) - Own: ${ownCharges}, Reimbursement: ${reimbCharges}`);
